<div @class(['form-group row align-items-center', 'has-error' => $errors->has($input), 'pb-3' => $paddingBottom])>
    ...
    <div class="col-lg-7 col-xl-6">
        <input id="{{ $input }}" name="{{ $input }}" type="number" class="form-control form-control-modern" value="{{ old($input, $value) }}" placeholder="{{ $placeholder }}" @if($required) required @endif {!! $data ?? null !!}>
        @isset($help)
